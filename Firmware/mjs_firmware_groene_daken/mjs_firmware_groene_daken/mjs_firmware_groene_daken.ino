...
//#define WITH_SPS30_I2C //OFF to enable room for GREENROOF in output

// pins for the green roof measurements
#define WITH_GREENROOF
uint8_t const SM_PIN = A1;
uint8_t const ST_PIN = A2;
...
#ifdef WITH_GREENROOF
    Serial.print(F("Roof M: "));
    Serial.println(soilM);
    Serial.print(F("Roof T: "));
    Serial.println(soilT);
#endif //WITH_GREENROOF
...
uint8_t extra_bits = 0;
  ...
  //send the measurement of Soil Temperature
  appendExtra(packet, soilT, 8);
...
uint16_t readSoilM(int samples){
  int c=0;
  float sum=0;
